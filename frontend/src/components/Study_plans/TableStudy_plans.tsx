...
import { loadColumns } from './configureStudy_plansCols';
...
const TableSampleStudy_plans = ({
  ...
  const {
    ...
    loading,
    count,
    notify: study_plansNotify,
    refetch,
  } = useAppSelector((state) => state.study_plans);
  ...
  useEffect(() => {
    if (study_plansNotify.showNotification) {
      notify(
        study_plansNotify.typeNotification,
        study_plansNotify.textNotification,
      );
    }
  }, [study_plansNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/study_plans/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/study_plans/study_plans-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/study_plans/study_plans-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
...
export default TableSampleStudy_plans;
